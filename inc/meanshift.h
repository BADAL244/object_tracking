...
const double CLUSTER_EPSILON = 2.0;
...
std::vector<Cluster> MeanShift::cluster(const std::vector<Point> &points,
                                        const std::vector<Point> &shifted_points)
{
    ...
    for (int i = 0; i < shifted_points.size(); i++) {
        ...
        if (c == clusters.size()) {
            Cluster clus;
            clus.mode = shifted_points[i];  // as the reference for this cluster
...
std::vector<Cluster> MeanShift::cluster(const std::vector<Point> &points, 
                                        double kernel_bandwidth)
{
    std::vector<Point> shifted_points = meanshift(points, kernel_bandwidth);
    return cluster(points, shifted_points);
}

std::vector<Cluster> meanshift_entry(const std::vector<LidarPoint> &points,
                                     double kernel_bandwidth)
{
    ...
    MeanShift meanshift;
    return meanshift.cluster(pts, kernel_bandwidth);
}